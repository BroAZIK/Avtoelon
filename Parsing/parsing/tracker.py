...
def tracker():
    ...
    for num_page in range(1, 2):
        html = get_page(f"https://avtoelon.uz/uz/avto/chevrolet/tracker/?page={num_page}")
        htmldom = BeautifulSoup(html.text, "lxml")
    

        items = get_items(htmldom)
        url = "https://azizbek4.pythonanywhere.com/api/cars/tracker/"
        fin_file = []
